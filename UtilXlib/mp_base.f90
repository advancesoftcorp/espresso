...
!
  ...
  INTEGER            :: info, myid
#if defined (_WIN32)
  INTEGER(KIND=i8b), ALLOCATE :: ps_sendbuf(:)
#endif
  !
#if defined __TRACE
  WRITE(*, *) 'reduce_base_integer IN'
  ...
  IF(root >= 0) THEN
    ...
    IF (myid == root) THEN
      ALLOCATE( ps_sendbuf( dim ) )
      ps_sendbuf = ps
      CALL MPI_REDUCE(ps_sendbuf, ps, dim, MPI_INTEGER8, MPI_SUM, root, comm, info)
  ...
  ELSE
    ALLOCATE( ps_sendbuf( dim ) )
    ps_sendbuf = ps
    CALL MPI_ALLREDUCE(ps_sendbuf, ps, dim, MPI_INTEGER8, MPI_SUM, comm, info)
