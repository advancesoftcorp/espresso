...
CONTAINS
  !
  !---------------------------------------------------------------------------
  SUBROUTINE kin_tf( length, rho, et, vt )
    !-------------------------------------------------------------------------
    !
...
!$omp parallel do default(shared) private(ir, r, r23)
    DO ir = 1, length
       ...
       IF ( r > rho_threshold ) THEN
          ...
          r23 = r ** (2.0_DP / 3.0_DP)
          !
          et(i) = et(i) + fact * C_tf * r * r23
          !
          vt(i) = vt(i) + fact * (5.0_DP / 3.0_DP) * C_tf * r23
          !
       END IF
       !
    END DO
!$omp end parallel do
  ...
  !---------------------------------------------------------------------------
    ...
    REAL(DP), INTENT(IN)    :: fact         ! scaling factor
    REAL(DP), INTENT(IN)    :: rho(length)  ! charge density
    REAL(DP), INTENT(IN)    :: grho(:)      ! gradient of rho: |\nabla rho|^2
    REAL(DP), INTENT(INOUT) :: et (length)  ! energy density             (added in place)
    ...
    REAL(DP) :: r, gr2
    !
!$omp parallel do default(shared) private(ir, r, gr2)
    DO ir = 1, length
       !
       r   = rho(i)
       gr2 = grho(i)
       ...
       IF ( r > rho_threshold ) THEN
          !
          et(i)  = et(i)  + fact * C_vw * gr2 / r
          !
          v1t(i) = v1t(i) - fact * C_vw * gr2 / (r * r)
          !
          v2t(i) = v2t(i) + fact * C_vw / r
          !
       END IF
       !
    END DO
