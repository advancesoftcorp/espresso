...
#if defined (_WIN32)
    INTEGER, ALLOCATABLE :: blacsmap_buf(:,:)
#endif
...
#if defined (_WIN32)
         ALLOCATE(blacsmap_buf(LBOUND(blacsmap,1):UBOUND(blacsmap,1), LBOUND(blacsmap,2):UBOUND(blacsmap,2)))
         blacsmap_buf = blacsmap
         CALL MPI_ALLREDUCE( blacsmap_buf, blacsmap,  SIZE(blacsmap), MPI_INTEGER, MPI_SUM, my_world_comm, ierr )
         DEALLOCATE(blacsmap_buf)
#else
