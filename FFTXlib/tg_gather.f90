...
#if defined (__MPI) && defined (_WIN32)
  REAL(DP), ALLOCATABLE :: tg_v_mpi_buf(:)
#endif
  INTEGER :: nxyp, ir3, off, tg_off
  INTEGER :: i, nsiz, ierr
...
!used to be   CALL mp_sum(tg_v, dffts%comm2 )
  ...
  tg_v_mpi = tg_v_d
#if defined (_WIN32)
  ALLOCATE(tg_v_mpi_buf(dffts%nnr_tg))
  tg_v_mpi_buf = tg_v_mpi
  CALL MPI_ALLREDUCE( tg_v_mpi_buf, tg_v_mpi, nsiz, MPI_DOUBLE_PRECISION, MPI_SUM, dffts%comm2, ierr )
  DEALLOCATE(tg_v_mpi_buf)
#else
...
#if defined (__MPI) && defined (_WIN32)
  COMPLEX(DP), ALLOCATABLE :: tg_v_mpi_buf(:)
#endif
  INTEGER :: nxyp, ir3, off, tg_off
  INTEGER :: i, nsiz, ierr
  INTEGER :: nr1x, nr2x, my_nr2p, my_i0r2p
