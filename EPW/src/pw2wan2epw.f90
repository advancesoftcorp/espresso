...
!    CALL phases_a_m()
  ...
  !      CLOSE(iummn)
...
#if defined (_WIN32)
    COMPLEX(KIND = DP), ALLOCATABLE :: wann_func_buf(:, :, :, :)
#endif
    !
    ...
    DO loop_w = 1, num_wannier_plot
      ...
      IF (meta_ionode) THEN
#if defined (_WIN32)
        ALLOCATE(wann_func_buf(LBOUND(wann_func,1):UBOUND(wann_func,1), &
                               LBOUND(wann_func,2):UBOUND(wann_func,2), &
                               LBOUND(wann_func,3):UBOUND(wann_func,3), &
                               LBOUND(wann_func,4):UBOUND(wann_func,4)))
        wann_func_buf = wann_func
        CALL MPI_REDUCE( wann_func_buf, wann_func, 2 * npol * ngridwf_max, MPI_DOUBLE_PRECISION, &
                         MPI_SUM, meta_ionode_id, world_comm, ierr )
        DEALLOCATE(wann_func_buf)
#else
