...
SUBROUTINE gram_schmidt_k( npwx, npw, nbnd, npol, psi, hpsi, spsi, e, &
  ...
  DO iblock = 1, nblock
     ...
     !
     CALL stop_clock( 'gsorthk:diag' )
     !
     ! ... Bcast diagonal block
     !
     CALL start_clock( 'gsorthk:bcast' )
     !
     ...
     !
     CALL stop_clock( 'gsorthk:bcast' )
     !
     ! ... Project off-diagonal block outside of diagonal block
     !
     CALL start_clock( 'gsorthk:offdiag' )
     !
     ...
     !
     CALL stop_clock( 'gsorthk:offdiag' )
     !
  END DO
  !
  ! ... Copy psi <- phi
  ...
  !
  CALL start_clock( 'gsorthk:esort' )
  !
  IF ( eigen_ )  CALL energyeigen( )
  !
  IF ( reorder ) CALL sort_vectors( )
  !
  CALL stop_clock( 'gsorthk:esort' )
  !
  DEALLOCATE( phi )
