...
SUBROUTINE diag_bands( iter, ik, avg_iter )
  ...
  ! ... Four types of iterative diagonalizations are currently used:
  ! ... a) Davidson algorithm (all-band)
  ! ... b) Conjugate Gradient (band-by-band)
  ! ... c) Projected Preconditioned Conjugate Gradient (block)
  ! ... d) RMM-DIIS algorithm (all-band)
  ! ...
  ! ... internal procedures :
  !
...
!             ! make sure that all processors have the same wfc
  ...
  SUBROUTINE diag_bands_k()
    ...
    END IF
    !
    !write (*,*) ' current isolve value ( 0 Davidson, 1 CG, 2 PPCG, 3 RMM)', isolve; FLUSH(6)
    IF ( isolve == 1 .OR. isolve == 2) THEN
       !
       ! ... (Projected Preconditioned) Conjugate-Gradient diagonalization
       !
