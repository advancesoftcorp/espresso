...
SUBROUTINE iosys()
  ...
  USE kinetic_module,        ONLY : do_kinetic, kinetic_dtdr, kinetic_weight, &
                                    kinetic_perturb, kinetic_nprint
  ...
  USE input_parameters, ONLY : title, calculation, verbosity, restart_mode,    &
                               ...
                               tkinetic, kin_dtdr, kin_weight, kin_perturb,    &
                               kin_nprint, max_xml_steps
...
11      CLOSE( UNIT = tempunit, STATUS = 'KEEP' )
  ...
  !
  ! ... set variables for SANNP
  !
  do_sannp = tsannp
  IF ( do_sannp ) CALL sannp_check()
  !
  ! ... set variables for Kinetic Energy Density
  !
  do_kinetic      = tkinetic
  kinetic_dtdr    = kin_dtdr
  kinetic_weight  = kin_weight
