...
SUBROUTINE print_clock_pw()
   ...
   ELSE IF ( isolve == 4 ) THEN
      ...
      ELSE
         ...
         END IF
      END IF
      WRITE( stdout, '(/5x,"Called by xwfcrot:")' )
      IF ( gamma_only ) THEN
         IF ( iverbosity > 0 )  THEN
      ...
      ELSE
         ...
         END IF
      END IF
      WRITE( stdout, '(/5x,"Called by gsorth:")' )
      IF ( gamma_only ) THEN
         IF ( iverbosity > 0 )  THEN
            CALL print_clock( 'gsorthg:diag' )
            CALL print_clock( 'gsorthg:bcast' )
            CALL print_clock( 'gsorthg:offdiag' )
            CALL print_clock( 'gsorthg:esort' )
         ENDIF
      ELSE
         IF ( iverbosity > 0 )  THEN
