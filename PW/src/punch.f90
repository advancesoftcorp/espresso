...
SUBROUTINE punch( what )
  ...
  USE klist,                ONLY : nks
  USE io_files,             ONLY : xmlpun_schema, psfile, molfile, pseudo_dir
  ...
  INTEGER            :: cp_status, nt, inlc, isolV
  !
  !
  ...
  IF (TRIM(what) == 'all') THEN 
     ...
     END IF
     !
     ! ... copy molecular files into the .save directory
     !
     IF ( lrism3d ) THEN
        !
        DO isolV = 1, nsolV
           cp_source = TRIM(pseudo_dir)//molfile(isolV)
           cp_dest   = TRIM(tmp_dir)//TRIM(prefix)//postfix//molfile(isolV)
           IF ( TRIM(cp_source) /= TRIM(cp_dest) ) &
                cp_status = f_copy(cp_source, cp_dest)
        END DO
        !
