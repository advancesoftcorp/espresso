...
SUBROUTINE electrons_scf ( printout, exxen )
  ...
  USE ener,                 ONLY : etot, hwf_energy, eband, deband, ehart, &
                                   vtxc, etxc, etxcc, ewld, demet, epaw, &
                                   elondon, edftd3, vsol, esol, egrand, &
                                   ef_up, ef_dw, exdm, ef
  ...
  CONTAINS
     ...
     SUBROUTINE print_energies ( printout )
       ...
       ELSE
          ...
          !
          IF ( lgcscf ) THEN
             WRITE( stdout, 9900 ) tot_charge
             WRITE( stdout, 9901 ) ef * RYTOEV, ABS( ef - gcscf_mu ) * RYTOEV
          END IF
          !
       ENDIF
       !
...
9900 FORMAT(/'     total charge of GC-SCF    =',0PF17.8,' e' )
9901 FORMAT( '     the Fermi energy          =',0PF17.8,' eV' &
            /'                        (error :',0PF17.8,' eV)')
