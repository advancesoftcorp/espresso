...
!------------------------------------------------------------------------------------------------
         ...
         IF ( lrism3d ) THEN
            ...
            END IF
            !
            output%rism3d_ispresent = .TRUE.
            CALL qexsd_init_rism3d(output%rism3d, nsolV, molfile, solvrho1, solvrho2, ecutsolv/e2)
            !
            DEALLOCATE( solvrho1 )
            DEALLOCATE( solvrho2 )
            !
            IF ( rism3d_is_laue() ) THEN
               output%rismlaue_ispresent = .TRUE.
               CALL qexsd_init_rismlaue(output%rismlaue, both_hands, laue_nfit, ireference, qsol, &
                                        starting_r, expand_r, buffer_r, buffer_ru, buffer_rv, &
                                        starting_l, expand_l, buffer_l, buffer_lu, buffer_lv)
               !
            ELSE
               output%rismlaue_ispresent = .FALSE.
               output%rismlaue%lwrite    = .FALSE.
            END IF
            !
         ELSE
            output%rism3d_ispresent = .FALSE.
            output%rism3d%lwrite    = .FALSE.
            !
            output%rismlaue_ispresent = .FALSE.
            output%rismlaue%lwrite    = .FALSE.
         END IF
...
!$omp end parallel
 ...
 100  CALL errore('pw_readschemafile',TRIM(errmsg),ierr)
    ...
    SUBROUTINE init_vars_from_schema( what, ierr, output_obj, par_info, gen_info, input_obj )
      ...
      IF ( ldim ) THEN
         ...
         CALL readschema_dim(par_info, output_obj%atomic_species, &
              ...
              output_obj%rism3d_ispresent,   output_obj%rism3d, &
              output_obj%rismlaue_ispresent, output_obj%rismlaue)
      ...
      END IF
      IF ( lef ) THEN
               CALL readschema_ef ( output_obj%band_structure, .FALSE. ) 
         !
      END IF
      ! 
      IF ( lpbc ) THEN
      ...
      !
      IF ( lconstmu ) THEN
         CALL readschema_ef ( output_obj%band_structure, .TRUE. )
      END IF
      !
      IF ( lrism .AND. output_obj%rism3d_ispresent ) THEN
         CALL readschema_rism3d ( output_obj%rism3d )
         !
         IF ( output_obj%rismlaue_ispresent ) THEN
            CALL readschema_rismlaue ( output_obj%rismlaue )
         END IF
      END IF
      !
      IF ( lrism_corr ) THEN
    ...
    SUBROUTINE readschema_dim(par_info_obj, atomic_species, atomic_structure, &
         ...
         rism3d_ispresent, rism3d, rismlaue_ispresent, rismlaue )
      !
    USE constants,        ONLY : e2
    ...
    USE qes_types_module, ONLY : parallel_info_type, atomic_species_type, atomic_structure_type, &
                                 symmetries_type, basis_set_type, band_structure_type, &
                                 rism3d_type, rismlaue_type, input_type
    IMPLICIT NONE 
    !
    ...
    LOGICAL, INTENT(IN)                        :: rism3d_ispresent
    TYPE ( rism3d_type ),INTENT(IN)            :: rism3d
    LOGICAL, INTENT(IN)                        :: rismlaue_ispresent
    TYPE ( rismlaue_type ),INTENT(IN)          :: rismlaue
    ! 
    INTEGER                                    :: npwx_
    ...
    !                                       3D-RISM
    !-------------------------------------------------------------------------
    IF (rism3d_ispresent) THEN
       lrism3d  = .TRUE.
       nsolV    = rism3d%nmol
       ecutsolv = rism3d%ecutsolv * e2
    ELSE
       lrism3d  = .FALSE.
       nsolV    = 0
       ecutsolv = 0.0_DP
    ...
    !                                       Laue-RISM
    !-------------------------------------------------------------------------
    IF (rismlaue_ispresent) THEN
       CALL rism3d_set_laue()
       both_hands = rismlaue%both_hands
       laue_nfit  = rismlaue%nfit
       expand_r   = rismlaue%right_expand
       expand_l   = rismlaue%left_expand
       starting_r = rismlaue%right_start
       starting_l = rismlaue%left_start
    ELSE
       both_hands = .FALSE.
       laue_nfit  = 0
       expand_r   = -1.0_DP
 ...
 !
    ...
    !
    !----------------------------------------------------------------------------------------
    SUBROUTINE readschema_ef ( band_struct_obj, with_nbnd )
    !----------------------------------------------------------------------------------------
       !
       USE constants, ONLY        : e2
       USE ener,  ONLY            : ef, ef_up, ef_dw
    ...
    !
    !----------------------------------------------------------------------------------------
    SUBROUTINE readschema_rism3d ( rism3d_obj )
    !----------------------------------------------------------------------------------------
       !
       USE constants,        ONLY : e2
       USE solvmol,          ONLY : nsolV, solVs
       USE rism3d_facade,    ONLY : lrism3d, ecutsolv
       USE io_files,         ONLY : molfile, pseudo_dir
       ...
       INTEGER :: isolV
       !
       IF ( nsolV /= rism3d_obj%nmol ) THEN
          CALL errore ("pw_readschema:", "pseudo_dir /= molec_dir", 1)
       END IF
       !
       ...
       END DO
       !
       ecutsolv = rism3d_obj%ecutsolv * e2
       !
    END SUBROUTINE readschema_rism3d
    !
    !----------------------------------------------------------------------------------------
    SUBROUTINE readschema_rismlaue ( rismlaue_obj )
    !----------------------------------------------------------------------------------------
       !
       USE rism3d_facade,    ONLY : qsol, laue_nfit, expand_r, expand_l, &
                                    starting_r, starting_l, buffer_r, buffer_ru, buffer_rv, &
       ...
       !
    END SUBROUTINE readschema_rismlaue
    !-----------------------------------------------------------------------------------  
